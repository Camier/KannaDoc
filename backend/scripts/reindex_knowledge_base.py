...
async def reindex_all():
    ...
    mongo_db = db.db
    if mongo_db is None:
        raise RuntimeError("Mongo database handle not initialized")
    ...
    kbs = await mongo_db.knowledge_bases.find({"is_delete": False}).to_list(length=None)
    ...
    for kb in kbs:
        ...
        print(
            f"Processing KB: {kb.get('knowledge_base_name')} ({kb_id}) - {len(files)} files"
        )
