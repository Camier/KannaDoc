...
async def setup_models(username: str = "thesis", dry_run: bool = False):
    ...
    print(
        f"{'[DRY RUN] ' if dry_run else ''}Setting up fresh models for user: {username}\n"
    )
    ...
    if existing:
        ...
        for model in existing.get("model_list", []):
            ...
            if url and (
                "172.17.0.1" in url or "litellm" in url.lower() or ":4000" in url
            ):
    ...
    if not dry_run:
        # Update or insert
        result = await collection.replace_one({"username": username}, doc, upsert=True)
