...
from app.rag.provider_registry import get_provider_for_model
    ...
    for conv in conversations:
        try:
            ...
            await db.db["conversations"].update_one(
                {"_id": conv["_id"]}, {"$set": {"model_config": model_config}}
            )

            logger.info(
                f"Migrated: {conv['_id']} ({old_model} -> provider: {provider})"
            )
            migrated += 1
...
async def verify_migration():
    ...
    with_provider = await db.db["conversations"].count_documents(
        {"model_config.provider": {"$exists": True}}
    )
    ...
    for conv in all_convs:
        ...
        if model_name:
            ...
            if provider != detected:
                logger.warning(
                    f"  Mismatch detected: {conv['_id']} - model: {model_name}, configured: {provider}, detected: {detected}"
                )
                invalid_count += 1
