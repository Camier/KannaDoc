...
async def main():
    ...
    print(
        f"Connecting to MongoDB: mongodb://{mongo_user}:****@{mongo_host}:{mongo_port}/{MONGODB_DB}?authSource=admin"
    )
    ...
    source_kb = await db.knowledge_bases.find_one(
        {"username": SOURCE_USERNAME, "knowledge_base_name": SOURCE_KB_NAME}
    )
            ...
            for file_doc in files_to_copy:
                ...
                try:
                    ...
                    await s3.copy_object(
                        CopySource=copy_source, Bucket=MINIO_BUCKET_NAME, Key=new_key
                    )
...
async def copy_milvus_collection(
    source_kb_id: str, target_kb_id: str, file_id_map: Dict[str, str]
):
    ...
    if count > 0:
        ...
        while True:
            ...
            if insert_data:
                ...
                print(
                    f"    Copied batch {len(insert_data)} entities (Last PK: {current_max_pk})"
                )
