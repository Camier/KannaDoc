...
if not MONGO_URL:
    ...
    if not mongodb_pass:
        raise ValueError("Either MONGODB_URL or MONGODB_PASSWORD must be set.")
    MONGO_PASS = urllib.parse.quote_plus(mongodb_pass)
    MONGO_URL = (
        f"mongodb://thesis:{MONGO_PASS}@mongodb:27017/chat_mongodb?authSource=admin"
    )
...
async def process():
    ...
    await update_task_progress(
        redis_conn, task_id, "processing", f"Processing {FILE_NAME}..."
    )
    ...
    await db["files"].update_one(
        {"file_id": FILE_ID}, {"$set": {"status": "parsed", "task_id": task_id}}
    )
