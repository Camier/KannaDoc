...
# Config (Internal Docker Network)
...
async def init_minio(session):
    async with session.client("s3", endpoint_url=MINIO_URL, aws_access_key_id=MINIO_ACCESS_KEY, aws_secret_access_key=MINIO_SECRET_KEY) as s3:
        try:
...
async def main():
    ...
    for jf in json_files:
        ...
        if not os.path.exists(pdf_path):
            logger.warning(f"⚠️ PDF not found for {jf}, skipping MinIO upload but ingesting vectors.")
        ...
        if os.path.exists(pdf_path):
            ...
            minio_url_path = minio_object_name # Store object key as url usually? Or full URL?
