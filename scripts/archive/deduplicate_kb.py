...
MONGO_USER = os.environ.get("MONGODB_ROOT_USERNAME")
MONGO_PASS = os.environ.get("MONGODB_ROOT_PASSWORD")
MONGO_HOST = "mongodb"  # Service name in docker-compose
MONGO_PORT = "27017"

if not MONGO_USER or not MONGO_PASS:
    print("Error: MONGODB_ROOT_USERNAME and MONGODB_ROOT_PASSWORD must be set.")
    sys.exit(1)
...
def delete_duplicates(token, duplicates):
    ...
    for filename, file_list in duplicates.items():
        ...
        for f in remove:
            ...
            delete_list.append({"knowledge_id": KB_ID, "file_id": f.get("file_id")})
