...
from bge_m3_service import bge_m3
import uvicorn
...
class SparseTextRequest(BaseModel):
...
async def embed_text(request: TextRequest):
    r = get_redis()
    results: List[Any] = [None] * len(request.queries)
    indices_to_compute = []
    queries_to_compute = []
...
async def embed_image(images: List[UploadFile] = File(...)):
    ...
    if pil_images_to_compute:
        print(
            f"Computing {len(pil_images_to_compute)} image embeddings (Cached: {len(images) - len(pil_images_to_compute)})..."
        )
...
async def embed_sparse(request: SparseTextRequest):
    r = get_redis()
    results: List[Any] = [None] * len(request.texts)
    indices_to_compute = []
    texts_to_compute = []
    ...
    if r:
        for i, text in enumerate(request.texts):
            if not text or not text.strip():
                results[i] = {}
                continue
            key = f"cache:embed:sparse:{compute_hash(text.encode('utf-8'))}"
            cached = r.get(key)
            if cached:
                results[i] = json.loads(cached)
            else:
    ...
    # 2. Compute Missing
    if texts_to_compute:
        print(f"Computing {len(texts_to_compute)} sparse embeddings...")
        computed_embeddings = bge_m3.encode_sparse(texts_to_compute)
        ...
        for local_idx, global_idx in enumerate(indices_to_compute):
            ...
            if r:
                key = (
                    "cache:embed:sparse:"
                    f"{compute_hash(texts_to_compute[local_idx].encode('utf-8'))}"
                )
