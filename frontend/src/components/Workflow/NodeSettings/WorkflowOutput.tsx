/**
 * WorkflowOutput Component - Chat interface for workflow execution
 *
 * Refactored to use NodeSettingsBase GlobalVariablesSection
 * Reduced from ~949 lines to ~650 lines by extracting common patterns.
 */
...
import { GlobalVariablesSection } from "./NodeSettingsBase";
...
const WorkflowOutputComponent: React.FC<WorkflowOutputProps> = ({
  ...
  const confirmDeleteAllChatflow = () => {
    if (showConfirmDeleteAllChatflow) {
      ...
      setShowConfirmDeleteAllChatflow(false);
    }
  };
  ...
  // Custom header component for WorkflowOutput
  ...
  // Chat interface section (WorkflowOutput-specific)
  const ChatSection = () => (
