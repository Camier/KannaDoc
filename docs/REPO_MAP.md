...
- `compose-clean` — wrapper script to sanitize environment
...
For solo projects or development environments, LAYRA provides a lightweight authentication alternative.

### Files
- `backend/app/core/simple_auth.py` — Simple auth module
- `backend/app/core/security.py` — Updated with simple auth support
- `backend/app/api/endpoints/auth.py` — Added `/login/apikey` endpoint

### Configuration (.env)
```bash
SIMPLE_AUTH_MODE=true          # Enable simple auth
SIMPLE_API_KEY=layra-dev-key-2024  # API key for X-API-Key header
...
| Token Storage | Redis + JWT | JWT only |
| Setup | Medium | Low |
